...
Defaults to true. Set to false to disable clicking to select a card.
...
Set to false to disable the button for expanding a single card for viewing at maximum size.
...
Default to false, which means that the user must select a card then click on its photo to show the
full-size image in a popup. If true, then the image pops up as soon as it is clicked.

**popupImageGrouping**

When an image is popped up showing the full size image as a page overlay, the overlay navigation
buttons allow the user to cycle through the images to display one at a time. This defaults to
